...
initEnv = fromList [("true", (Boolean True)),
                    ...
                    ("fn", (Fn lispFn)),
                    ("macro", (Fn lispMacro)),
                    ("cons", (Fn (stdFn lispCons))),
                    ("first", (Fn (stdFn lispFirst))),
                    ("rest", (Fn (stdFn lispRest))),
                    ("empty?", (Fn (stdFn lispEmpty))),
                    ("if", (Fn lispIf)),
                    ("list", (Fn (stdFn lispList))),
                    ("quote", (Fn lispQuote))]
...
lispMacro ((SExpr args):body) env =
  return (Fn (\ params callEnv -> do 
                 fnEnv <- mergeMacroArgs args params env
                 form <- evalBody body fnEnv
...
main :: IO ()
main = do
  let program = wrapStdLib "(let (x 1 y 2) (+ x y))"
